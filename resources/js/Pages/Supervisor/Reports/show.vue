<template>
  <AuthenticatedLayout title="Report Details">
    <template #header>
      <div class="flex justify-between items-center">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">Report Details</h2>
        <Link :href="route('supervisor.reports.index')" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">← Back to Reports</Link>
      </div>
    </template>
        ...
        <div class="bg-white shadow sm:rounded-lg p-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
              ...
              <p class="text-lg font-medium text-gray-900">{{ report.report_type || 'General' }}</p>
            </div>
            <div>
...
const approveReport = () => {
  ...
  form.post(route('supervisor.reports.update', props.report.id), {
